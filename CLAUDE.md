# ArtSync — iPad Art Data Infrastructure
...
cargo build -p artsync-crypto
cargo build -p artsync-api-server
cargo build -p artsync-cli

# Run all tests
cargo test

# Run tests for a specific crate
cargo test -p artsync-crypto
...
S3_BUCKET=test API_KEY=test cargo run -p artsync-api-server

# Run the CLI
cargo run -p artsync-cli -- --help
```
